...
- `registerPlugin(plugin)` with capability declarations (`providers`, `hooks`)
...
- `listPluginSummaries()` and `getPluginSummary(pluginId)` for plugin metadata (api version, owned provider counts, registered hooks)
